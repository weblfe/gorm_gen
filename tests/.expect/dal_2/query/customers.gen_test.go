...
func init() {
	...
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&model.Customers{}) fail: %s", err)
	}
}

func Test_customersQuery(t *testing.T) {
	customers := newCustomers(db)
	customers = *customers.As(customers.TableName())
	_do := customers.WithContext(context.Background()).Debug()

	primaryKey := field.NewString(customers.TableName(), clause.PrimaryKey)
	...
	_, ok := customers.GetFieldByName("")
	if ok {
		t.Error("GetFieldByName(\"\") from customers success")
	}

	err = _do.Create(&model.Customers{})
	if err != nil {
		t.Error("create item in table <customers> fail:", err)
	}

	err = _do.Save(&model.Customers{})
	if err != nil {
		t.Error("create item in table <customers> fail:", err)
	}

	err = _do.CreateInBatches([]*model.Customers{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <customers> fail:", err)
	}

	_, err = _do.Select(customers.ALL).Take()
	if err != nil {
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*model.Customers{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
		t.Error("FindInBatches() on table <customers> fail:", err)
	}

	_, err = _do.Select(customers.ALL).Where(primaryKey.IsNotNull()).Order(primaryKey.Desc()).Find()
	if err != nil {
	...
	_, err = _do.Select(customers.ALL).Omit(primaryKey).Take()
	if err != nil {
	...
	_, err = _do.ScanByPage(&model.Customers{}, 0, 1)
	if err != nil {
