...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&model.CreditCards{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&model.CreditCards{}) fail: %s", err)
	}
}

func Test_creditCardsQuery(t *testing.T) {
	creditCards := newCreditCards(db)
	creditCards = *creditCards.As(creditCards.TableName())
	_do := creditCards.WithContext(context.Background()).Debug()

	primaryKey := field.NewString(creditCards.TableName(), clause.PrimaryKey)
	...
	_, ok := creditCards.GetFieldByName("")
	if ok {
		t.Error("GetFieldByName(\"\") from creditCards success")
	}

	err = _do.Create(&model.CreditCards{})
	if err != nil {
		t.Error("create item in table <credit_cards> fail:", err)
	}

	err = _do.Save(&model.CreditCards{})
	if err != nil {
		t.Error("create item in table <credit_cards> fail:", err)
	}

	err = _do.CreateInBatches([]*model.CreditCards{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <credit_cards> fail:", err)
	}

	_, err = _do.Select(creditCards.ALL).Take()
	if err != nil {
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*model.CreditCards{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
		t.Error("FindInBatches() on table <credit_cards> fail:", err)
	}

	_, err = _do.Select(creditCards.ALL).Where(primaryKey.IsNotNull()).Order(primaryKey.Desc()).Find()
	if err != nil {
	...
	_, err = _do.Select(creditCards.ALL).Omit(primaryKey).Take()
	if err != nil {
	...
	_, err = _do.ScanByPage(&model.CreditCards{}, 0, 1)
	if err != nil {
